Scenario: Verify DELETE /api/company/part/{id}/ deletes a single supplier part
Meta:
@api
@apiVersion 479
@endpoint DELETE /api/company/part/{id}/
@responseCode 204
...
!-- Create a supplier part to delete
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
...
Scenario: Verify DELETE /api/company/part/ bulk deletes supplier parts
Meta:
@api
@apiVersion 479
@endpoint DELETE /api/company/part/
@responseCode 204
When I save access token for user with username `${username}` and password `${password}` to scenario variable `token`
!-- Create prerequisite supplier company
Given I initialize scenario variable `companyName` with value `#{generate(Company.name)} #{generate(regexify '[a-z]{5}')}`
...
Then response code is equal to `200`
When I save JSON element value from `${response}` by JSON path `$.results[0].pk` to scenario variable `partId`
!-- Create a supplier part to bulk delete
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
...
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"items": [${supplierPartId}]}
When I execute HTTP DELETE request for resource with URL `${main-page-url}api/company/part/`
Then response code is equal to `204`
Then response does not contain body
!-- Cleanup company
When I add request headers:

...
@api
@apiVersion 479
@endpoint GET /api/part/{id}/requirements/
@responseCode 200

Scenario: Verify GET /api/part/{id}/requirements/ returns part requirements
When I save access token for user with username `${username}` and password `${password}` to story variable `token`
!-- Create a part to get requirements for
...
|name         |value          |
|Authorization|Token ${token} |
When I execute HTTP GET request for resource with URL `${main-page-url}api/part/${partId}/requirements/`
Then response code is equal to `200`
Then JSON element value from `${response}` by JSON path `$.total_stock` is greater than or equal to `0`
Then JSON element value from `${response}` by JSON path `$.unallocated_stock` is greater than or equal to `0`

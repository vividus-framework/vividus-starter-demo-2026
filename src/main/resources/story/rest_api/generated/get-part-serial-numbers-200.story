...
@api
@apiVersion 479
@endpoint GET /api/part/{id}/serial-numbers/
@responseCode 200

Scenario: Verify GET /api/part/{id}/serial-numbers/ returns serial number info
When I save access token for user with username `${username}` and password `${password}` to story variable `token`
!-- Create a trackable part to get serial numbers for
Given I initialize scenario variable `partName` with value `#{generate(regexify 'PART-[a-z0-9]{8}')}`
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"name": "${partName}", "description": "#{generate(Lorem.sentence)}", "active": true, "trackable": true}
...
|name         |value          |
|Authorization|Token ${token} |
When I execute HTTP GET request for resource with URL `${main-page-url}api/part/${partId}/serial-numbers/`

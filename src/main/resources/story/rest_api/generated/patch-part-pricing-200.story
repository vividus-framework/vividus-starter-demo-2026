...
Scenario: Verify PATCH /api/part/{id}/pricing/ partially updates pricing overrides
When I save access token for user with username `${username}` and password `${password}` to story variable `token`
!-- Create a part to patch pricing for
...
!-- Partially update pricing via PATCH
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"override_min": "#{randomInt(1, 50)}.00", "override_min_currency": "USD"}
When I execute HTTP PATCH request for resource with URL `${main-page-url}api/part/${partId}/pricing/`

Scenario: Verify POST /api/company/part/manufacturer/ creates a new manufacturer part
Meta:
@api
@apiVersion 479
@endpoint POST /api/company/part/manufacturer/
@responseCode 201
...
Given I initialize scenario variable `description` with value `#{generate(Lorem.sentence)}`
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"part": ${partId}, "manufacturer": ${manufacturerId}, "MPN": "${mpn}", "description": "${description}"}
...
Then JSON element value from `${response}` by JSON path `$.description` is equal to `${description}`

...
Scenario: Verify POST /api/part/category/parameters/ creates a category parameter template
...
Given I initialize scenario variable `defaultValue` with value `#{generate(regexify '[a-z0-9]{5}')}`
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"category": ${categoryId}, "template": ${templateId}, "default_value": "${defaultValue}"}
...
Then JSON element value from `${response}` by JSON path `$.default_value` is equal to `${defaultValue}`

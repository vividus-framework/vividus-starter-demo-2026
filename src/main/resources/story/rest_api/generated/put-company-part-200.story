Scenario: Verify PUT /api/company/part/{id}/ updates a supplier part
Meta:
@api
@apiVersion 479
@endpoint PUT /api/company/part/{id}/
@responseCode 200
...
!-- Create a supplier part to update
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
...
!-- Update with PUT
Given I initialize scenario variable `updatedSku` with value `#{generate(regexify '[A-Z]{3}-[0-9]{6}')}`
Given I initialize scenario variable `updatedDescription` with value `#{generate(Lorem.sentence)}`
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"part": ${partId}, "supplier": ${supplierId}, "SKU": "${updatedSku}", "description": "${updatedDescription}"}
When I execute HTTP PUT request for resource with URL `${main-page-url}api/company/part/${supplierPartId}/`
Then response code is equal to `200`
Then JSON element value from `${response}` by JSON path `$.SKU` is equal to `${updatedSku}`

...
@api
@apiVersion 479
@endpoint PUT /api/part/category/parameters/{id}/
@responseCode 200

Scenario: Verify PUT /api/part/category/parameters/{id}/ updates a category parameter template
...
!-- Update via PUT
Given I initialize scenario variable `updatedDefault` with value `#{generate(regexify '[a-z0-9]{5}')}`
When I add request headers:
|name         |value           |
|Authorization|Token ${token}  |
|Content-Type |application/json|
Given request body: {"category": ${categoryId}, "template": ${templateId}, "default_value": "${updatedDefault}"}
When I execute HTTP PUT request for resource with URL `${main-page-url}api/part/category/parameters/${catParamId}/`
Then response code is equal to `200`
Then JSON element value from `${response}` by JSON path `$.pk` is equal to `${catParamId}`
Then JSON element value from `${response}` by JSON path `$.default_value` is equal to `${updatedDefault}`
!-- Cleanup
When I add request headers:
|name         |value          |
|Authorization|Token ${token} |
